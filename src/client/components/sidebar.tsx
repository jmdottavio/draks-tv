  ...
  return (
    <div className="relative shrink-0 group/avatar">
      <img
        ...
        className={`rounded-full ring-2 ${ringColor} ${glowClass} transition-all duration-200 ${
  ...
  if (channel.isLive) {
    return (
      <div className="flex items-center gap-2 text-sm">
        <span className="font-bold text-live">{formatViewers(channel.viewerCount ?? 0)}</span>
...
function SectionHeader({ title, count, isExpanded }: { title: string; count: number; isExpanded: boolean }) {
  if (!isExpanded) {
    return (
...
function ChannelList({ channels, isExpanded, onFavoriteToggle }: ChannelListProps) {
  ...
  if (channels?.length === 0) {
    return (
      <div className={`py-8 text-center text-sm text-sidebar-text-dim ${!isExpanded && 'px-1'}`}>
  ...
  return (
    <div className="space-y-2">
      {liveChannels.length > 0 && (
        <div>
          <SectionHeader title="Live" count={liveChannels.length} isExpanded={isExpanded} />
      ...
      {offlineChannels.length > 0 && (
        <div>
          <SectionHeader title="Offline" count={offlineChannels.length} isExpanded={isExpanded} />
          <div className="space-y-0.5">
