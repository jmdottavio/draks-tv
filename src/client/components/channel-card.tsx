        ...
        <div className="flex-1 min-w-0">
          ...
          {channel.latestVod !== null && (
            <span className="text-sm text-text-dim">
              {formatDuration(channel.latestVod.duration)} · {formatDate(channel.latestVod.createdAt)}
            </span>
          )}
        </div>
    ...
    >
      ...
      <div className="p-4">
        ...
        {channel.isLive && channel.stream !== null && (
          <>
            <div className="text-sm text-text-primary mb-2 line-clamp-2" title={channel.stream.title}>
              {channel.stream.title}
            </div>
            <div className="flex items-center gap-3 text-sm text-text-muted mb-4">
              <span>{channel.stream.gameName}</span>
              <span className="text-live font-semibold">{formatViewers(channel.stream.viewerCount)} viewers</span>
            </div>
          </>
        )}
        ...
        {!channel.isLive && channel.latestVod !== null && (
          <>
            <div className="text-sm text-text-primary mb-2 line-clamp-2" title={channel.latestVod.title}>
              {channel.latestVod.title}
            </div>
            <div className="flex items-center gap-3 text-sm text-text-muted mb-4">
              <span className="text-text-dim">{formatDuration(channel.latestVod.duration)}</span>
