...
function getLastSeenDate(twitchId: string): string | null {
  const row = db.query<ChannelLastSeenRow, [string]>(
...
function getAllLastSeenDates(): Map<string, string> {
  const rows = db.query<ChannelLastSeenRow, []>(
    'SELECT twitch_id, last_seen_at FROM channel_last_seen'
  ).all();
...
function setLastSeenFromVod(twitchId: string, login: string, displayName: string, vodDate: string): void {
  db.run(
    `INSERT INTO channel_last_seen (twitch_id, login, display_name, last_seen_at, updated_at)
     VALUES (?, ?, ?, ?, datetime('now'))
     ON CONFLICT(twitch_id) DO UPDATE SET
